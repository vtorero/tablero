...
function DashboardTable({asistenciasdetalles={}}) {
    const columns = [
        ...
        // {
        //     header: "VIGENTE",
        //     accessorKey: 'i_vigente',
        //     footer: ({ table }) => table.getFilteredRowModel().rows.reduce((total, row) => total + row.getValue('i_vigente'), 0),
        // },
        {
            header: "VIGENTE ACUMULADO",
            accessorKey: 'vigente_acumulado',
            //footer: ({ table }) => table.getFilteredRowModel().rows.reduce((total, row) => total + row.getValue('vigente'), 0),
        },
    ]
